...
files=["c104.txt","r101.txt","rc208.tex"]
for date_text in files:
    date_text = "./datefile/" + date_text
    ...
    for rou in range(1):
        ...
        import os
        # ディレクトリの名前を指定
        d1 = date_text.replace(".txt","")
        dd = d1.replace("./datefile/","")
        base_directory_name = f"{dd}_A1_output_files"
        ...
        #全車両から交渉の提案を受け付ける
            ...
            negotiation_list = []
            negotiation_id = 0
            # print(f"Offer_listの長さ{len(Offer_list)}")
        ...
        filename = os.path.join(directory_name, "1log_main.txt")
        min_CVN = min(log_CVN)
        CVN_index = log_CVN.index(min_CVN)
        min_CRT = min(log_CRT)
        CRT_index = log_CRT.index(min_CRT)
        with open(filename, 'a') as f:
            f.write(f"steps {CVN_index} CVN {min_CVN}\n")
            f.write(f"steps {CRT_index} CRT {min_CRT}\n")
