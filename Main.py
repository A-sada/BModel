...
for datefile in files:
    date = datefile
    datefile = os.path.join(directory_path, datefile)
    run_num = 0
    ...
    second_directory_name = os.path.join(base_directory_name, current_time_str)


    # ディレクトリが存在しない場合、作成（親ディレクトリも含めて）
    os.makedirs(second_directory_name, exist_ok=True)
    directory_name = os.path.join(second_directory_name, date)
    os.makedirs(directory_name, exist_ok=True)

    ll=[]
    ll=read_task(datefile,tasks)
    max_xy = ll[0]
    max_time = ll[1]
    #n = (int)(max_xy /25)
